...
/** 左侧导航响应 **/
 ...
 /**当搜索框为空回复全部列表**/
	...
	$("div#updatePortrait span.sucServer").click(function(){
	 ...
	 $("#user_personalInfo .user_personalInfo_wrap .lj-btn").click(function(){
	 	alert("ok");
	 	$.post('/user/modify_user',{'name':$("input#username").val(),'sex':1,
	 		'age':$("input#age").val() == '' ? 0 : $("input#age").val(),'contact':$("input#phone").val(),'timestamp':$("input#timestamp").val()},
	 			function(result){
	 			if(result.code == 200){
	 				alert("个人信息修改成功");
	 				$(".user-main .main-left .user-name span#span_username").text(result.userEntity.name);
	 				$(".header .wrapper .fr .login a .log").text(result.userEntity.name);
	 				$(".user-main .tabbable div#tab2 span#user_name").text(result.userEntity.name);
	 				$(".user-main .tabbable div#tab2 span#user_sex ").text(result.userEntity.sex);
	 				$(".user-main .tabbable div#tab2 span#user_age").text(result.userEntity.age);
	 				$(".user-main .tabbable div#tab2 span#user_contact").text(result.userEntity.contact);
	 				$(".user-main .tabbable div#tab2 span#user_date").text(result.userEntity.timestamp);
