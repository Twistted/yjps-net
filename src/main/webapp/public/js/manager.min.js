...
/** 左侧导航响应 **/
 ...
 /**当搜索框为空回复全部列表**/
	...
	/** 我的发布列表刷新 **/
	$(".user-main .main-left ul li a#li_5").click(function(){
		alert("asd");
<<<<<<< HEAD
		// $.post('/agent/list_house', function(result){
		// 	if (result.code == 200) {
		// 		for(var i = 0 ;  i < result.houseList.size; i++){
		// 			$("#table").append("<tr id='result.houseList[i].houseNumber'>
		// 									<td>result.houseList[i].houseNumber</td>
		// 									<td>result.houseList[i].villageName</td>
		// 									<td>result.houseList[i].roomAmount+R+result.houseList[i].livingroomAmount+L+result.houseList[i].toiletAmount+T+result.houseList[i].balconyAmount+B</td>
...
=======
		$.post('/agent/list_house', function(result){
			...
			if (result.code == 200) {
				$("#house_list tbody tr").remove();
				for(var i = 0 ;  i < result.houseListSize; i++){
				 
					$("#house_list").append("<tr id='" + result.houseList[i].houseNumber + "' >"
									   + "<td>" + result.houseList[i].houseNumber + "</td>"
									   + "<td>" + result.houseList[i].villageName + "</td>"
									   + "<td>" + result.houseList[i].roomAmount + "室" + result.houseList[i].livingroomAmount + "厅" + result.houseList[i].toiletAmount + "卫" + result.houseList[i].balconyAmount + "阳</td>"
									   + "<td>" + result.houseList[i].downPayment + "</td>"
									   + "<td>" + result.houseList[i].prize + "</td>"
									   + "</tr>")
				}
...
>>>>>>> e2452ba15fc230899389aa3612992c210ae9430a
	});
	 $(function () {
        var cur_totalpage=9999;
        //分页
        var fetchConfigData = function (curr) {
            $.post('/agent/list_house', {'page': curr || 1}, function (result) {
                createTable(result);
        ...
        var createTable = function (result) {
        	...
        	for(var i = 0 ; i < result.houseListSize;i++){
        		$("#table_list").append("<tr id='$(result.houseList[i].houseNumber)'>"+
        									"<td>" + result.houseList[i].houseNumber + "</td>"+
        									"<td>" + result.houseList[i].villageName + "</td>"+
        									"<td>" + "一厅两房" + "</td>"+
        									"<td>" + result.houseList[i].downPayment + "</td>"+
