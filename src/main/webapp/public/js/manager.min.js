 ...
 /**当搜索框为空回复全部列表**/
	...
	/**表格搜索**/
     ...
     $('#search-bt').click(function(){
 	 ...
 	 var objUrl;
 	 /** upload pic **/
 	 $("#picUploadClientFile").change(function(){
	 objUrl = getObjectURL(this.files[0]) ;
	console.log("objUrl = "+objUrl) ;
	if (objUrl) {
		$("#img_preview").attr("src", objUrl) ;
		$("#img_preview_small").attr("src", objUrl) ;
		$("#img_preview_middle").attr("src", objUrl) ;
		$("#img_preview_big").attr("src", objUrl) ;
	...
	$("div#updatePortrait span.sucServer").click(function(){
	 ...
	 $(".field").change(function(){  
                    if(($("input.field").val())!=""){
                      ...
                      if(($("input.field").val())==""||($("input.field").val())==" "){
	 		...
	 		'introduction':$("textarea#selfIntro").val()},function(result){
	 			if(result.code == 200){
					...
					window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.success);
	 				...
	 				$("header .wrapper .fr .login a .log_name").text(result.agentEntity.name);
	 ...
	 $("#user_personalInfo .user_personalInfo_wrap .user_personalInfo_btn").click(function(){
	 	$.post('/user/modify_user',{'name':$("input#username").val(),'sex':$("#sex_select").val(),
	 		'age':$("input#age").val() == '' ? 0 : $("input#age").val(),'contact':$("input#phone").val()},function(result){
	 			var sex_string = "男"
	 			if(result.userEntity.sex == 1){
	 				sex_string = "男"
	 			}
	 			else{
	 				sex_string = "女"
	 			...
	 			if(result.code == 200){
					...
					window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.success);
	 					$("#user_left_username").text(result.userEntity.name);
		 			 	...
		 			 	$(".info_c .user_sex").text(sex_string);
	 ...
	 $(".tabbable .user-main .tab-content .fy .fabu .lj-btn").click(function(){
	 	if($("#FyTitle").val() == ''){
	 		...
	 		if($("#houseTypeUrlId").val() != '' && $("#houseViewUrlId").val() != ''){
	 			$.post('/agent/add_house',{'title':$("input#FyTitle").val() == '' ? 0 : $("input#FyTitle").val(),
				'houseNumber':$("input#houseNumber").val() == '' ? 0 : $("input#houseNumber").val(),
				...
				'address':$("input#FyAddress").val(),'state':$("select#state").val(),
				'houseTypeUrl':$("#houseTypeUrlId").val(),
				'houseViewUrl':$("#houseViewUrlId").val()},function(result){	 
					if(result.code==200){
						var txt=  "房源发布成功";
					window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.success);
					}
    ...
    var createTable = function (result) {
    	...
    	for(var i = 0 ; i < result.houseListSize;i++){
    		$("#table_list tbody").append("<tr id='" + result.houseList[i].houseId + "'>" +
    									"<td>" + "<a href='" + "/house/house?id=" + result.houseList[i].houseId + "' >" + result.houseList[i].houseNumber + "</a>" + "</td>" +
    									...
    									"<td>" + "<span class='actionForList actionForList_del' id='" + result.houseList[i].houseId + "'>" + "删除" + "</span>" + "</td>" +
    											// "<span>" + " | " + "</span>" + "<span class='actionForList actionForList_add'>" + "<a href='#add_pic' id='list_addPic+[i]'>"+"添加" +"</a>"+ "</span>" + "</td>"+
    	...
    	// 	$.post("/house/house"),{'id':$(this).attr("value")},function(result){
    	// 		if(result.code == 200){
    	...
    	// });
    	/** 删除 **/
    	$(".actionForListdel_del").click(function(){
    		if(confirm("确定删除吗？")){
	    ...
	    $(function(){
		  ...
		  $('#list_addPic').leanModal({ top: 110, overlay: 0.45, closeButton: ".hidemodal" });
		});
